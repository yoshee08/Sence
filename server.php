...
$result1  = mysql_query("SELECT * FROM `players` WHERE `server_id` = '$server_id'");
$players = mysql_fetch_array($result1, MYSQL_ASSOC);
?>
...
<?php 
    $favicon = file_get_contents ('http://minecraft-api.com/api/ping/favicon.php?ip=' . $server_data['ip'] . '&port=' . $server_data['port'] . ''); 
    
if($status == 0){
	echo '<div class="alert alert-important">';
	echo $favicon;'<p style="font-size: 24px;margin-top: 7px;"><strong>' . $server_data['ip'] . '</strong> is offline</p>';
	echo '</div>';
	//die();
}?>	
...
<ul class="nav nav-pills" id="tabs" style="height: 100px;" >
...
<div class="tab-content" >	
	<div class="tab-pane active" id="info">
		<table class="table table-bordered pull-left" style="/*width: 40%;*/ background:white;">
<?php 
if($status == 0){
...
<h3><p class="/*pull-right*/">Comments (<?php echo mysql_result(mysql_query("SELECT COUNT(`id`) FROM `comments` WHERE `server_id` = '$server_id'"), 0);?>) <?php if(logged_in() == true){ ?><span class="label label-success" style="cursor:pointer;" onClick="$('#comments').toggle('slow');">Add Comment</span><?php } ?></p></h3>
<br>
